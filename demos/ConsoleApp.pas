...
{02/12/01 Adapted for Kol by Thaddy de Koning,
 ...
 I replaced Tstrings with pStrList and all
 Exceptions raised into MsgOk! ;-) not very safe huh?
 But Martin was so kind to write his code in such a way
 that most pitfalls wil still be overcome.(And I'm a lazy bastard)
 You will obtain the last windows error as a Number.
 This should be replace by a win32 formatmessage call later.
 Also, I used Kol's own routines to recognize winversion
 instead of the SYSUtils ones.
 The whole conversion cost me ten minutes of thought and
 two minutes to do: be warned, no guarantees.
 *******************************************
 DO NOT USE THIS ON NONE 32 BIT EXECUTABLES,
 *******************************************
 i.e. COMMAND.COM is a NoNo!}
...
function ExecConsoleApp(const ApplicationName,
                        Parameters: String;
                        AppOutput: pStrList;     {will receive output of child process}
                        OnNewLine: TOnEvent  {if assigned called on each new line}
                        ): DWORD;
{Parameters
  ApplicationName.
...
begin
  ...
  try
    ...
    if (ImageDosHeader.e_magic <> IMAGE_DOS_SIGNATURE) then {not executable}
      MsgOk('Dos signature not present');
    ...
    begin
      ...
      if (ImageFileHeader.Characteristics and IMAGE_FILE_EXECUTABLE_IMAGE) = 0 then
        MsgOk('File is not executable');  {could be COFF obj}
...
begin
  ...
  if Newline then
    ...
    Items[Count-1]:= LineBuf; {should never happen with count = 0}
  Newline:= false;
  LineBufPtr:= 0;
...
begin
  ...
  if (Ext = '.BAT') or ((Winver = wvNT) and (Ext = '.CMD')) then
  begin {just have a bash}
    Commandline:=Format( '"%s" %s', [ApplicationName, Parameters])
  end else
  ...
  begin
    if SearchPath(nil, PChar(ApplicationName), ExeExt, SizeOf(AppNameBuf), AppNameBuf, ExeName) = 0 then
      MsgOk(Format('Could not find file %s', [ApplicationName]));
    if Ext = ComExt then
    ...
    if ((BinType = SCS_DOS_BINARY) or (BinType = SCS_DPMI_BINARY)) and
        (WinVer = wvNT) then
      CommandLine:=Format( 'cmd /c""%s" %s"', [AppNameBuf, Parameters])
    else
    if (BinType = SCS_32BIT_BINARY) and (SubSyst = IMAGE_SUBSYSTEM_WINDOWS_CUI) then
      CommandLine:=Format( '"%s" %s', [AppNameBuf, Parameters])
    else
      MsgOk('Executable image is not a supported type')
            {Supported types are Win32 Console or MSDOS under Windows NT only}
  end else
  begin
    MsgOk(Format('%s has invalid file extension', [ApplicationName]));
  end;
...
{$ENDIF}
  ...
  if not CreatePipe(ReadHandle, WriteHandle, @SecurityAttributes, 0) then
   MsgOk(Int2STr(GetLastError));//RaiseLastWin32Error;
